...
@section('content')
    ...
    <form action="{{ route( 'admin.car-type.list' ) }}" method="post" x-list-form="main">
        <div class="row">
            <div class="col-md-12">
                <!--begin::Portlet-->
                <div class="m-portlet">
                                ...
                                <table class="table table-striped m-table m-table--head-separator-metal" x-list-table>
                                    <thead>
                                        ...
                                        <tr>
                                            ...
                                            <th>
                                                @if( \App\Http\Controllers\Admin\AdminController::CAN( 'carType.add' ) )
...
@section('templates')
    ...
    <script type="text/template" x-tr>
        <tr x-tr-id="<%= rc.id %>">
            ...
            <td>
                <span class="m-switch">
                    <label>
                        <input x-activate-url="{{ route('admin.car-type.activate') }}" type="checkbox" <% if ( rc.is_active == 1 ){ %> checked="checked" <% } %> @if( ! \App\Http\Controllers\Admin\AdminController::CAN( 'carType.activate' ) ) disabled @endif>
                        <span></span>
                    </label>
                </span>
            </td>
            <td>
                @if( \App\Http\Controllers\Admin\AdminController::CAN( 'carType.edit' ) )
                    <button type="button" class="btn btn-sm m-btn btn-primary m-btn--icon m-btn--icon-only" x-edit-url="{{ route('admin.car-type.get') }}">
                        <i class="m-menu__link-icon flaticon-edit"></i>
                    </button>
                @endif
                @if( \App\Http\Controllers\Admin\AdminController::CAN( 'carType.delete' ) )
                    <button type="button" class="btn btn-sm m-btn btn-dark m-btn--icon m-btn--icon-only" x-delete-url="{{ route('admin.car-type.delete') }}">
    ...
    <script type="text/template" x-edit-modal>
        <div class="m-portlet">
            <form action="{{ route( 'admin.car-type.edit' ) }}" class="m-form m-form--fit m-form--label-align-right" x-edit-form>
